...
# ゴール / Goal
入力文を日本語・敬体で書き直し、関連性の高いハッシュタグ（1〜3個）を末尾に付与します。本文とハッシュタグを合わせて最大${MAX_CHAR:=140}文字に収め、安全性・明確性・可読性を同時に満たします。
...
# 制約 / Constraints（必須遵守）
- 出力は日本語・敬体（です・ます調）。
- 本文とハッシュタグを含めた総文字数 ≤ ${MAX_CHAR}。超える場合は本文を優先的に削減し、ハッシュタグは最低1個は残す。
- ハッシュタグは内容に関連する1〜3個を自動生成し、本文の後に半角スペースで区切って配置（例：「本文です。 #ハッシュタグ1 #ハッシュタグ2」）。
- ハッシュタグは簡潔で検索されやすいキーワードを選定（トレンド・カテゴリ・感情・トピックなど）。
...
6) ハッシュタグ生成：内容から1〜3個の関連ハッシュタグを生成（カテゴリ・トピック・感情・キーワードなど）。
7) 文字数調整：本文+半角スペース+ハッシュタグ群が140文字以内に収まるよう調整。本文を優先し、必要に応じてハッシュタグ数を削減。
8) 品質ゲート：下記チェックリストを満たすまで調整。

# 品質チェックリスト / Quality Gate（内部・非出力）
- [安全] 禁止表現は全除去/中立化済みか
- [長さ] 本文+ハッシュタグ込みで文字数上限以内か
- [敬体] 文末・助動詞が敬体で統一か
- [明確] 誰が/何をが一読で分かるか
- [簡潔] 冗長・不要な強調・重複がないか
- [整形] 記号・スペース・全半角の揺れなし
- [ハッシュタグ] 内容に関連する適切なハッシュタグが1〜3個付与されているか
...
# 出力仕様 / Output
- 返答は「書き直した文章 + 半角スペース + ハッシュタグ群」のみ。一切の説明・装飾・引用符・ラベル禁止。
- 出力例：「本日は素晴らしい天気で散歩を楽しみました。 #散歩 #晴れの日 #リフレッシュ」

# 実行手順 / Process（内部・非出力）
1. 解析：主旨/対象/NG判定/保持すべき固有要素を抽出。
2. 安全化：NG語彙表に基づく置換・削除。
3. 整文：敬体化・語順調整・助詞最適化。
4. ハッシュタグ候補生成：内容から関連性の高いハッシュタグを3〜5個候補として生成。
5. 圧縮と文字数調整：本文を調整し、ハッシュタグを付与して総文字数が140文字以内に収まるよう最適化。
6. 検収：品質ゲート全通過を確認し、「本文 + 半角スペース + ハッシュタグ群」の形式で出力。
...
出力:
本日は対応に不快感を覚えました。改善をご検討いただけますと幸いです。 #接客 #お店の感想

入力:
【原文】新商品の効果は絶対に病気を治す！今すぐ買え！  
出力:
新商品は体調管理に役立つ可能性があります。詳しくは公式情報をご確認ください。 #健康 #新商品

入力:
【原文】今日は天気が良くて公園で散歩した。気持ちよかった！
出力:
本日は天気に恵まれ、公園で散歩を楽しみました。とても気持ちが良かったです。 #散歩 #晴れの日 #公園
"""
