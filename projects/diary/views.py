...
class GoogleCalendarAPI:
    ...
    def authenticate(self):
        ...
        token_path = os.path.join(settings.BASE_DIR, 'projects', 'token.pickle')
        credentials_path = os.path.join(settings.BASE_DIR, 'projects', 'credentials.json')
    ...
    def get_events_by_date(self, target_date=None, max_results=50):
        """指定した日付のカレンダーイベントを取得"""
        if not self.service:
            return []
    ...
    def format_events(self, events):
        ...
        for event in events:
            ...
            start_time = ""
            end_time = ""
            if 'T' in str(start):  # dateTimeの場合
                start_dt = datetime.fromisoformat(start.replace('Z', '+00:00'))
                start_time = start_dt.strftime('%H:%M')
            if 'T' in str(end):  # dateTimeの場合
...
def index(request):
    ...
    # 今日の予定を取得
    today_events = calendar_api.get_events_by_date()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            calendar_context = ""
            if calendar_events:
                calendar_context = "今日の予定:\n"
                for event in calendar_events:
                    time_info = ""
                    if event['start_time'] and event['end_time']:
                        time_info = f"({event['start_time']}-{event['end_time']})"
                    elif event['start_time']:
                        time_info = f"({event['start_time']})"
                    
                    calendar_context += f"・{event['title']} {time_info}\n"
                    if event['location']:
                        calendar_context += f"  場所: {event['location']}\n"
                calendar_context += "\n"
            ...
            def wiki_get_data(url):
                ...
                genres_text = ""
                if genre_element:
                    plainlist_div = genre_element.find("div", class_="plainlist")
                    if plainlist_div:
                    ...
                    messages=[
                        {
                            ...
                            ※今日の予定がある場合は、その文脈も考慮して日記的な要素も加えてください。
                            ...
                            - カレンダーの予定情報がある場合は、それも考慮した内容にしてください
                            """
...
def get_calendar_events(request):
    ...
    if request.method == 'POST':
        try:
            ...
            calendar_api = GoogleCalendarAPI()
            events = calendar_api.get_events_by_date(target_date)
    ...
    return HttpResponse(json.dumps({'success': False, 'error': 'Invalid request'}), 
                       content_type='application/json')
